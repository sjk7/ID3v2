...
namespace Detail {
    ...
    // Should be preferably called fully qualified, like
...
struct IMutableRegistryHub {
    ...
    virtual void registerTagAlias(std::string const& alias,
        std::string const& tag, SourceLineInfo const& lineInfo)
        = 0;
...
#pragma clang diagnostic ignored "-Wnon-virtual-dtor"
        ...
        // support. use of const for Object pointers is very uncommon and under
...
#pragma clang diagnostic pop
        ...
        template <typename T>
        MatchAnyOf<T> MatcherBase<T>::operator||(
            MatcherBase const& other) const {
...
namespace Matchers {
    ...
    namespace Vector {
        ...
        struct EqualsMatcher : MatcherBase<std::vector<T, AllocMatch>> {
            ...
            bool match(std::vector<T, AllocMatch> const& v) const override {
                // !TBD: This currently works if all elements can be compared
                // using !=
                // - a more general approach would be via a compare template
                // that defaults to using !=. but could be specialised for, e.g.
...
namespace Generators {
    ...
    // Tag type to signal that a generator sequence should convert arguments to
    // a specific type
...
namespace Generators {
    ...
    template <typename T> class RepeatGenerator : public IGenerator<T> {
        ...
        bool next() override {
            ...
            // If it happily moves, we are left in that state, otherwise it is
            // time to start reading from our cache
            if (m_current_repeat == 0) {
                const auto success = m_generator.next();
                if (!success) {
                    ++m_current_repeat;
...
namespace Generators {
    ...
    GeneratorWrapper<ResultType> from_range(
    ...
    GeneratorWrapper<ResultType> from_range(Container const& cnt) {
        return GeneratorWrapper<ResultType>(
            pf::make_unique<IteratorGenerator<ResultType>>(
                cnt.begin(), cnt.end()));
    }
...
namespace Detail {
    ...
    inline std::string getAnnotation(Class cls,
        std::string const& annotationName, std::string const& testCaseName) {
        NSString* selStr =
...
namespace Benchmark {
    namespace Detail {
        ...
        /// We need to reinvent std::function because every piece of code that
        /// might add overhead in a measurement context needs to have consistent
        /// performance characteristics so that we can account for it in the
        /// measurement. Implementations of std::function with optimizations
        /// that aren't always applicable, like small buffer optimizations, are
        /// not uncommon. This is effectively an implementation of std::function
        /// without any such optimizations; it may be slow, but it is
        /// consistently slow.
...
namespace Benchmark {
    namespace Detail {
        ...
        TimingOf<Clock, Fun, int> measure_one(
            Fun&& fun, int iters, std::false_type) {
...
namespace Benchmark {
    namespace Detail {
        ...
        Estimate<double> bootstrap(double confidence_level, Iterator first,
            ...
            auto a = [bias, accel](
                         double b) { return bias + b / (1. - accel * b); };
            double b1 = bias + z1;
            double b2 = bias - z1;
            double a1 = a(b1);
            double a2 = a(b2);
        ...
        bootstrap_analysis analyse_samples(double confidence_level,
            int n_resamples, std::vector<double>::iterator first,
            std::vector<double>::iterator last);
    } // namespace Detail
} // namespace Benchmark
} // namespace Catch
...
namespace Benchmark {
    namespace Detail {
        ...
        SampleAnalysis<Duration> analyse(const IConfig& cfg,
            Environment<Duration>, Iterator first, Iterator last) {
        ...
        template <typename Clock = default_clock> void run() {
            ...
            CATCH_TRY {
                ...
                BenchmarkStats<FloatDuration<Clock>> stats{info,
                    analysis.samples, analysis.mean,
...
namespace TestCaseTracking {
    ...
    class ITracker {
        ...
        virtual ITrackerPtr findChild(NameAndLocation const& nameAndLocation)
            = 0;
    ...
    namespace TextFlow {
        ...
        class Column {
            ...
            inline friend std::ostream& operator<<(
                std::ostream& os, Column const& col) {
                bool first = true;
                for (auto line : col) {
                    if (first)
                        first = false;
...
namespace clara {
    namespace detail {
        ...
        // Transport for raw args (copied from main args, or supplied via init
        // list for testing)
        class Args {
            friend TokenStream;
        ...
        inline auto convertInto(std::string const& source, bool& target)
            ...
            std::transform(
                srcLC.begin(), srcLC.end(), srcLC.begin(), [](unsigned char c) {
        ...
        template <typename L> struct BoundFlagLambda : BoundFlagRefBase {
            ...
            auto setFlag(bool flag) -> ParserResult override {
                return LambdaInvoker<
                    typename UnaryLambdaTraits<L>::ReturnType>::invoke(m_lambda,
                    flag);
            }
        };
        ...
        struct Parser : ParserBase {
            ...
            auto parse(
                std::string const& exeName, TokenStream const& tokens) const
                -> InternalParseResult override {
                ...
                // ParserInfo parseInfos[totalParsers]; // <-- this is what we
                // really want to do
...
clara::Parser makeCommandLineParser(ConfigData& config) {
    ...
    auto cli = ExeName(config.processName) | Help(config.showHelp)
        ...
        | Opt(
            setWaitForKeypress, "never|start|exit|both")["--wait-for-keypress"](
...
namespace Detail {
    ...
    namespace {
        ...
        StringRef extractInstanceName(StringRef enumInstance) {
            ...
            return enumInstance.substr(
                name_start, enumInstance.size() - name_start);
        }
    } // namespace
...
static LONG CALLBACK handleVectoredException(
...
#pragma clang diagnostic pop
        ...
        bool WithinRelMatcher::match(double const& matchee) const {
            const auto relMargin = m_epsilon
...
namespace Matchers {
    ...
    namespace StdString {
        ...
        std::string StringMatcherBase::describe() const {
            std::string description;
            description.reserve(5 + m_operation.size()
                + m_comparator.m_str.size()
    ...
    struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
        ...
        static GeneratorTracker& acquire(TrackerContext& ctx,
            ...
            if (currentTracker.nameAndLocation() == nameAndLocation) {
                ...
                tracker
                    = std::static_pointer_cast<GeneratorTracker>(childTracker);
            } else {
...
namespace {
    ...
    class ConsoleAssertionPrinter {
        ...
        ConsoleAssertionPrinter(std::ostream& _stream,
            AssertionStats const& _stats, bool _printInfoMessages)
            : stream(_stream)
            , stats(_stats)
            , result(_stats.assertionResult)
            , colour(Colour::None)
...
namespace {
    std::string getCurrentTimestamp() {
        ...
        // issues Also, UTC only, again because of backward compatibility (%z is
        // C++11)
        time_t rawtime;
        std::time(&rawtime);
        auto const timeStampSize = sizeof("2017-01-16T17:06:45Z");
...
extern "C" int CATCH_INTERNAL_CDECL wmain(
    int argc, wchar_t* argv[], wchar_t*[]) {
#else
