...
{
    virtual std::streamsize getSize(bool refresh = false) const noexcept override
    {
        if (refresh || m_size <= 0)
        {
    ...
    virtual std::streampos getPos(bool refresh = false) const noexcept override
    {
        if (m_pos == 0 || refresh)
        {
            m_pos = m_f.tellg();
    ...
    // and is ok.
    virtual std::streamoff seek(std::streamoff pos, std::ios::seekdir dir = std::ios::beg) noexcept override
    {
    ...
    {
        auto read = utils::file_read_some(m_buf, m_f, nBytes, fs::path{m_filePath});
        if (read >= 0)
            m_pos += read;
    ...
    virtual size_t readInto(std::string &s, size_t nBytes) override
    {
        try
        {
            auto sz = static_cast<size_t>(utils::file_read_some(s, m_f, nBytes, m_filePath));
            m_pos += sz;
            return sz;
        }
        catch (std::exception &)
    ...
    virtual const std::string &data() const noexcept override
    {
        return m_buf;
    }

    // throws system_error on failure, hopefully with a meaningful message.
    FileDataReader(const std::string &filePath, unsigned int openFlags = std::ios::in | std::ios::binary)
        : m_filePath(filePath)
    {
