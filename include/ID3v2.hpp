...
// we use a *signed* type here, because it may be used for seeking backward
static constexpr inline std::streamsize FRAME_HEADER_SIZE = sizeof(TagExtendedHeader);
...
static inline std::string ValidityToString(const verifyTagResult &v, const TagHeader &h)
{

    unsigned int a = static_cast<unsigned int>(v);
    unsigned int highest = static_cast<unsigned int>(verifyTagResult::OK);
    unsigned int lowest = static_cast<unsigned int>(verifyTagResult::BadReservedFlags);
    ...
    switch (v)
    {
    case verifyTagResult::OK:
        return "No Verification Error";
    case verifyTagResult::BadBase:
        return "Unknown Verification Error";
    case verifyTagResult::BadVersion:
        return "Bad ID3v2(.3) version";
    case verifyTagResult::BadID:
        return "Bad ID: [" + std::string(h.ID, 3) + "]";
    case verifyTagResult::BadSizeIndicator:
        return "Bad Size Indicator";
    case verifyTagResult::BadReservedFlags:
        return "Bad reserved flags";
    default:
        return "Out of range verification error";
    }
}
template <typename T, size_t size> std::string fromFixed(const T (&array)[size])
{
...
struct TagHeaderEx : TagHeader
{
    ...
    std::string v1Artist() const noexcept
    {
        return fromFixed(v1Tag.artist);
    }
    std::string v1Title() const noexcept
    {
        return fromFixed(v1Tag.title);
    }

    std::string v1Year() const noexcept
    {
        return fromFixed(v1Tag.year);
    }
    std::string v1Album() const noexcept
    {
        return fromFixed(v1Tag.album);
    }
    std::string v1Comment() const noexcept
    {
        return fromFixed(v1Tag.comment);
    }
    unsigned char v1Genre() const noexcept
    {
        return v1Tag.genre;
    }
    int v1year() const noexcept
    {
        return std::atoi(v1Year().c_str());
    }
...
struct ID3FileInfo
{
    ...
    const TagHeaderEx &Tag() const noexcept
    {
        return m_tag;
    }
...
struct FrameHeaderEx : FrameHeader
{
  ...
  public:
    FrameHeaderEx(const ID3FileInfo &info, const std::streamoff &filePos) : m_info(info), filePosition(filePos)
    {
    }
    ...
    auto SizeInBytes(bool includingHeader = false)
    {
        ...
        {
            throw std::runtime_error(m_info.FilePath() + "Frame size is more than the entire file!");
        }
        return includingHeader ? sizeInBytes + sizeof(FrameHeader) : sizeInBytes;
    }
    ...
    size_t ReadData(my::IDataReader &dr)
    {
        ...
        if (dr.getPos() != loc)
        {
            ...
            if (seeked != loc)
            {
                throw std::runtime_error(m_info.FilePath() + " cannot seek to data start position");
            }
        }

        const auto sz = SizeInBytes();
        allData.resize((size_t)(sz));
        const auto rd = static_cast<size_t>(dr.readInto(allData.data(), (size_t)sz));
        if (rd != sz)
        {
            throw std::runtime_error(m_info.FilePath() + " failed to read all the data from the file");
        }
        return allData.size();
    }

    // non syncsafe in 2.3, synchsafe in 2.4
    static inline size_t GetFrameSize(const FrameHeader &hdr, bool syncsafe = false)
    {
    ...
    const std::string &AllData() const noexcept
    {
        return allData;
    }
...
{
    auto sz = swapEndian(szInd); // v2.3: for the tag, it's sync safe (probably!) lol
    auto ret = decodeSynchSafe(sz);
    return ret;
}

static inline TagHeaderEx parseHeader(my::IDataReader &dr, const std::string &filePath)
{
    ...
    if (data.size() == 10)
    {
        ...
        {
            throw std::runtime_error(filePath + "v2 Tag is not valid" + ValidityToString(ret.validity, ret));
        }
    }
...
template <typename CB> static inline void FillTags(ID3FileInfo &info, my::IDataReader &dr, CB &&cb)
{
    ...
    const std::streamoff dataStartPos =
        tag.hasExtendedHeader ? sizeof(TagHeader) + sizeof(TagExtendedHeader) : sizeof(TagHeader);
    ...
    if (dr.getPos() != dataStartPos)
    {
        ...
        if (pos != dataStartPos)
        {
            throw std::runtime_error(info.FilePath() + ": could not seek to data start position");
        }
        curPos = pos;
    }
    ...
    while (dr.getPos() < endPos)
    {
        ...
        else
        {
            ...
            if (remain_tag > 0)
            {
                ...
                const auto justPaddingRemains = std::all_of(padding.begin(), padding.end(), [](const char c) {
                    if (c == '\0')
                        return true;
                    return false;
                });
            ...
            MYTRACE(MyTraceFlags::defaults, info.FilePath(), "Have invalid ID3v2 Frame Header at file position: ", pos);
        }
    }
}
...
struct AbstractFrame
{
    AbstractFrame(const FrameHeaderEx &h) : m_hdr(h)
    {
        ...
        if (raw.empty())
            throw std::runtime_error("Abstract Frame: no data");
    }
    virtual ~AbstractFrame() = default;
    FrameHeader hdr{};
    std::string data;
    std::string &allData() noexcept
    {
        return data;
    }
    FrameHeaderEx m_hdr;
...
struct CommentsFrame : AbstractFrame
{
    CommentsFrame(const FrameHeaderEx &h) : AbstractFrame(h)
    {
        ...
        const auto nNulls = this->textEncoding == TextEncodingType::ansi ? 1 : 2;
        ...
        if (found == std::string::npos)
        {
            ...
            if (std::all_of(this->payLoad.begin(), this->payLoad.end(), [](const char c) { return c != 0; }))
            {
                this->malFormed = true;
                return; // good enuf
            }
            else
            {
                ...
                this->payLoad.clear();
                throw std::runtime_error("CommentsFrame: no null found to indicate where the "
...
struct TagCollection
{
  ...
  public:
        ...
        TagDataReader rdr(filepath, [&](const FrameHeaderEx &h) {
            ...
            if (h.frameID[0] == 'T')
            {
                ...
                if (h.IDString() == "TXXX")
                {
                    ...
                    if (ptr)
                        m_tags.insert({ptr->uid(), std::move(ptr)});
                    // handled = true;
                }
                else
                {
    ...
    const ContainerType &Tags() const noexcept
    {
        return m_tags;
    }
